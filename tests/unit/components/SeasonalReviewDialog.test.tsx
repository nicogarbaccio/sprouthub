...
describe("SeasonalReviewDialog", () => {
  ...
  describe("rendering", () => {
    ...
    it("should not render when dialog is closed", () => {
      ...
      expect(
        screen.queryByText("Spring Schedule Review")
  ...
  describe("schedule changes display", () => {
    ...
    it("should display change direction icons correctly", () => {
      const { container } = render(<SeasonalReviewDialog {...defaultProps} />);
    ...
    it("should show reasoning for each suggestion", () => {
      ...
      expect(
        screen.getByText("Spring growing season needs more frequent watering")
      ).toBeInTheDocument();
      expect(
        screen.getByText("Temperature cooling down in fall")
      ).toBeInTheDocument();
      expect(screen.getByText("Hot summer weather")).toBeInTheDocument();
      expect(
        screen.getByText("Outdoor plant - more sensitive to seasonal changes")
      ).toBeInTheDocument();
    });
    ...
    it("should display previous schedule performance when available", () => {
      ...
      expect(
        screen.getByText("15 days (good performance)")
      ).toBeInTheDocument();
    });
  });
  ...
  describe("applied suggestions display", () => {
    it("should show applied state for completed suggestions", () => {
      ...
      const monsteraCard = screen
        .getByText("Monstera Deliciosa")
        .closest(".opacity-75");
      ...
      const hasCheckIcon = Array.from(checkIcons).some(
        (icon) =>
          (icon as SVGElement).classList.contains("text-green-500") ||
          (icon as SVGElement).classList.contains("text-green-600")
      );
  ...
  describe("loading states", () => {
    ...
    it("should show loading state in apply all button", () => {
      const { container } = render(
        <SeasonalReviewDialog {...defaultProps} isLoading={true} />
      );
    ...
    it("should work with all season types", () => {
      ...
      seasons.forEach((season) => {
        const { unmount } = render(
          <SeasonalReviewDialog {...defaultProps} season={season} />
        );
        const expectedTitle =
          season.charAt(0).toUpperCase() + season.slice(1) + " Schedule Review";
