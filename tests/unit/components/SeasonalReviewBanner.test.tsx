...
describe("SeasonalReviewBanner", () => {
  ...
  describe("rendering", () => {
    ...
    it("should display correct plant count with singular form", () => {
      render(
        <SeasonalReviewBanner {...defaultProps} plantsNeedingReview={1} />
      );
    ...
    it("should display correct plant count with plural form", () => {
      render(
        <SeasonalReviewBanner {...defaultProps} plantsNeedingReview={5} />
      );
    ...
    it("should display triggering factors when available", () => {
      ...
      render(
        <SeasonalReviewBanner {...defaultProps} transition={transition} />
      );

      expect(screen.getByText("Detected changes:")).toBeInTheDocument();
      expect(
        screen.getByText(
          /Daylight hours below 12 hours.*Temperature below 18°C/
        )
      ).toBeInTheDocument();
    });
  ...
  describe("season-specific styling and icons", () => {
    ...
    it.each([
      ...
      "should display $season styling and emoji",
      ({ season, fromSeason, emoji, textClass, bgClass, borderClass }) => {
        ...
        expect(
          screen.getByText(`${emoji} ${seasonCapitalized} has arrived!`)
        ).toBeInTheDocument();
  ...
  describe("confidence level styling", () => {
    ...
    it.each([
      ...
      "should display $confidence confidence with appropriate styling",
      ({ confidence, bgClass, textClass }) => {
        const transition = createMockTransition(
          "summer",
          "fall",
          confidence as any
  ...
  describe("accessibility", () => {
    ...
    it("should have accessible button labels", () => {
      ...
      expect(
        screen.getByRole("button", { name: /review schedules/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /1 week/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /2 weeks/i })
      ).toBeInTheDocument();
    });
  ...
  describe("visual elements", () => {
    it("should display appropriate icons", () => {
      ...
      const iconClasses = Array.from(icons).map(
        (icon) => (icon as SVGElement).className
      );
      const hasSeasonIcon = iconClasses.some(
        (className) =>
          typeof className === "string" && className.includes("h-6 w-6")
      );
      const hasDropletsIcon = iconClasses.some(
        (className) =>
          typeof className === "string" && className.includes("h-4 w-4")
      );
  ...
  describe("edge cases and data handling", () => {
    it("should handle zero plants needing review", () => {
      render(
        <SeasonalReviewBanner {...defaultProps} plantsNeedingReview={0} />
      );
    ...
    it("should handle large numbers of plants", () => {
      render(
        <SeasonalReviewBanner {...defaultProps} plantsNeedingReview={100} />
      );
    ...
    it("should handle long triggering factors list", () => {
      ...
      expect(
        screen.getByText(
          /Very long triggering factor.*Another factor.*Third factor/
        )
      ).toBeInTheDocument();
    });
    ...
    it("should work with mock seasonal transitions from test utils", () => {
      render(
        <SeasonalReviewBanner
          {...defaultProps}
          transition={mockSeasonalTransitions.winterToSpring}
        />
      );
